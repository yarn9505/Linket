	...
	public void insertBoardDaumAttach(BoardDTO boardDTO) throws Exception {
		...
		if (boardDTO.getImgPath() != null && boardDTO.getImgPath().length > 0) {
			...
			for (int i = 0; i < boardDTO.getImgPath().length; i++) {
				...
				// path의 경로를 바꾼다.
				String orgImgPathName = tmpImgPathName.replace(UPLOAD_DAUM_EDITOR_IMAGE_TEMP_PATH,
						UPLOAD_BOARD_IMG_PATH);
				...
				// 파일 복사 작업 (템프 --> 원본 폴더로)
				File boardImgTempFile = new File(FILE_UPLOAD_PATH + tmpImgPathName); // 임시경로의
																						// 파일
																						// 생성

				String boardImgPath = UPLOAD_BOARD_IMG_PATH + "/" + boardImgTempFile.getName();

				File boardImgFile = new File(FILE_UPLOAD_PATH + "/" + boardImgPath); // 실제
																						// 경로의
																						// 파일
																						// 생성
		...
		if (boardDTO.getFilePath() != null && boardDTO.getFilePath().length > 0) {
			...
			for (int i = 0; i < boardDTO.getFilePath().length; i++) {
				...
				// path의 경로를 바꾼다.
				String orgFilePathName = tmpFilePathName.replace(UPLOAD_DAUM_EDITOR_FILE_TEMP_PATH,
						UPLOAD_BOARD_FILE_PATH);
	...
	public void updateBoardDaumAttach(BoardDTO boardDTO) throws Exception {
		...
		if (boardDTO.getImgPath() != null && boardDTO.getImgPath().length > 0) {
			...
			for (int i = 0; i < boardDTO.getImgPath().length; i++) {
				...
				// path의 경로를 바꾼다.
				String orgImgPathName = tmpImgPathName.replace(UPLOAD_DAUM_EDITOR_IMAGE_TEMP_PATH,
						UPLOAD_BOARD_IMG_PATH); // 실제경로로 교체
				...
				// String boardImgPath = UPLOAD_BOARD_IMG_PATH + "/" +
				// boardImgTempFile.getName();
		...
		if (boardDTO.getFilePath() != null && boardDTO.getFilePath().length > 0) {
			...
			for (int i = 0; i < boardDTO.getFilePath().length; i++) {
				...
				// String boardFilePath = UPLOAD_BOARD_FILE_PATH + "/" +
				// boardFileTempFile.getName();
