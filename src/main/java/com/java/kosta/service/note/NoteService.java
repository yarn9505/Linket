...
public interface NoteService {
	...
	// 2-3) sender_del과 recv_del 이 모두 Y 인 경우 해당 튜플을 delete하는 쿼리 // 둘 다 Y 인경우만
	// 삭제되고 각 보낸보관함, 받는 보관함에서 삭제한 경우 Y로 업데이트만!
	...
	// 4-2) 쪽지를 읽는 순간 받은이가 읽은시간을 now()로 갱신(update)하고 받은이가 읽었는지 유무(recv_read)를 Y로
	// update(수정)하는 쿼리
